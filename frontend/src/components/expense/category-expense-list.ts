...
export class CategoryExpensList {
    ...
    private async getCategories(): Promise<void> {
        const response: PatternResponseType = await CategoryExpenseServices.getCategories();
    ...
    private setLinkForCategoryDeleted(e: Event): void {
        if(this.confirmationDeleteBtnElement && e.target){
            this.confirmationDeleteBtnElement.href = '/expenses-category-delete?id=' + (<HTMLElement>e.target).getAttribute('data-id');
        }
    }
}